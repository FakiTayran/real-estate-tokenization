...
function App() {
  ...
  const handleBuy = async () => {
    if (propertyId && estateAgentId && !!property) {
      ...
      SetTransactionReceipt(receipt.transactionHash);
      SetSuccess(true);
    }
  };
  return (
    <main className="grid place-items-center h-screen max-w-6xl mx-auto">
      <section className="container mx-auto flex flex-col gap-8">
        {success ? (
          <SuccessScreen transactionHash={transactionReceipt} />
        ) : (
          <div>
            <h1 className="text-transparent bg-gradient-to-br from-primary/20 to-primary bg-clip-text text-[5rem] text-center">
              Property Sale
            </h1>
            {!!property && (
              <Table>
                    ...
                    {isLoading ? (
                      ...
                      <>
                        ...
                        <TableCell className="text-right">
                          {abbreviateAddress(
                            (property as PropertyDetails).owner
                          )}
                        </TableCell>
                      </>
                    )}
